-- Query: generate_d_customer_table
-- Purpose: Generate the d_customer table by transforming and joining source tables based on the provided mapping document.
-- Query Type: SELECT
...
WITH base_data AS (
    SELECT
        ...
        ad_mstr.ad_fax,
        ad_mstr.ad_line1,
        ad_mstr.ad_line2 AS ad_line2_dup,
        ad_mstr.ad_line3,
        ad_mstr.ad_phone,
        cm_mstr.cm_mod_date,
        ad_mstr.ad_type AS ad_type_dup,
        currency.currencydescription,
        d_cntry_hier.cntry_hier_key
    FROM tfsdl_aig_qad_delta.cm_mstr cm_mstr
    LEFT JOIN tfsdl_aig_qad_delta.ad_mstr ad_mstr
        ON cm_mstr.cm_addr = ad_mstr.ad_addr
        AND LOWER(cm_mstr.cm_domain) = LOWER(ad_mstr.ad_domain)
    LEFT JOIN tfsdl_aig_qad_delta.currency currency
        ON currency.currencycode = cm_mstr.cm_curr
    JOIN tfsdl_edp_common_dims.d_cntry_hier d_cntry_hier
