...
--   Pattern: dimension_flat
-- Query 8: f_purchase_order_transformation
--   Pattern: fact_union_dedup
-- Query 9: d_supplier_transformation
--   Pattern: dimension_flat
...
-- Pattern: dimension_flat
-- Coverage: 139 rows
-- WARNING: unresolved items: Join condition for sap_dim_org_unit_site is unclear. Placeholder used.
...
-- =============================
/**************************************************************************
This plan transforms data for the d_gl_acct dimension table.
It includes mappings from dim_gl_acct and sap_dim_gl_acct.
**************************************************************************/
...
SELECT
  ...
  coalesce(sga.ktopl, CAST(NULL AS string)) AS coa_id,
...
LEFT JOIN sap_dim_gl_acct sga ON dga.ccompany_uuid = sga.tcompany_uuid
...
This plan handles the transformation of f_purchase_order using a union of current and history tables.
It includes deduplication logic and field mappings as specified.
**************************************************************************/
...
SELECT
  ...
  po.KCQUANTITY * po.KCNET_PRC_AMT AS open_txn_amt,
...
coalesce((EKPO.BPUMZ/IF(EKPO.BPUMN=0,1,EKPO.BPUMN)),0))) as double) AS unit_prc_txn_amt,
  ...
  CASE WHEN fct.cprocess_type = 'THPA' THEN 'Y' ELSE 'N' END AS vomi_flag,
...
WHERE po.CPO_ID IS NOT NULL AND po.CITM_ID IS NOT NULL AND fct.po_nbr IS NOT NULL
...
No specific pattern detected, using generic transformation.
**************************************************************************/
...
WHERE fct.CINVOICE_UUID IS NOT NULL AND fct.CSELLER IS NOT NULL
